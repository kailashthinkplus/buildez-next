...
function CanvasPlayer({ manifest, progress, fit = "cover" }: { manifest: CinematicManifest; progress: number; fit?: Fit }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      drawFrame(canvasARef.current, imagesRef.current[targetIndex]);
...
export default function CinematicSequence({ manifest, progress, scrollLength = "300vh", fit = "cover", className }: Props) {
  if (prefersReducedMotion()) {
    ...
    const src = resolveFramePath(manifest.desktopFallback ?? manifest.desktop, mid);
    return (
  ...
  if (progress !== undefined) {
    return (
      <div className={className}>
        <CanvasPlayer manifest={manifest} progress={progress} fit={fit} />
      </div>
    );
  }
  ...
  return (
    <StickyScene length={scrollLength} className={className}>
      {(p) => <CanvasPlayer manifest={manifest} progress={p} fit={fit} />}
    </StickyScene>
  );
}
