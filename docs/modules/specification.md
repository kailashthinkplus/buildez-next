...
Specification emits `WebsiteSpec` and related SDK contracts. The spec is the central contract consumed by constraints, reasoning, Decision Engine, compiler, mapper, simulation, critic, repair, learning, and ai-v10 orchestration.

WebsiteSpec should be built from `BusinessIntelligenceProfile`, `BrandIntelligenceProfile`, `ContentStrategy`, `ExperienceStrategy`, and `PatternIntelligenceResult`.
