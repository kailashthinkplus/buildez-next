...
export async function POST(req: NextRequest) {
  ...
  if (provider === "COD") { await reserveInventory(lines); sendShopOrderEmails(order, items, shop); return NextResponse.json({ orderId: order.id, orderNumber: order.orderNumber, provider, status: "confirmed" }); }
