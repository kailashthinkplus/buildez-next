...
export async function POST(req: NextRequest) {
  ...
  const visitorHash = crypto.createHmac("sha256", process.env.ANALYTICS_HASH_SECRET || process.env.AUTH_SECRET || "buildez-analytics").update(`${ip}:${clean(body.visitorId, 80)}`).digest("hex");
