...
export async function GET(req: NextRequest) {
  ...
  const [products, orders, customers, revenue, recent, paidOrders, topProducts, topCustomers] = await Promise.all([
  ...
  return NextResponse.json({ shop: access.shop, metrics: { products, orders, customers, revenue: Number(revenue._sum.total || 0), dailySales: days, topProducts: topProducts.map(item => ({ title: item.title, units: item._sum.quantity || 0, revenue: Number(item._sum.total || 0) })), topCustomers: customerLeaders }, recent });
}
