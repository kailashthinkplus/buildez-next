...
import { Clock3, Loader2, MailPlus, ShieldCheck, Trash2, UserPlus, UserRound, Users } from "lucide-react";
import AddTeamMemberModal from "./components/AddTeamMemberModal";
import MemberActionsMenu from "./components/MemberActionsMenu";
import type { Invite, Member, Person, Role } from "./types";
...
export default function TeamPage() {
  ...
  async function onInvited(text: string) {
  ...
  return <div className="mx-auto max-w-6xl pb-12">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-medium text-blue-600 dark:text-blue-400">Workspace access</p><h1 className="mt-1 text-3xl font-semibold tracking-[-.04em]">Team</h1><p className="mt-2 text-sm dashboard-muted">Invite collaborators and control what they can manage.</p></div><div className="flex items-center gap-3"><div className="rounded-xl border dashboard-border px-4 py-2 text-sm"><strong>{data?.limits.used ?? 0}</strong><span className="dashboard-muted"> / {data?.limits.total ?? 0} seats used</span></div>{data?.canManage ? <button onClick={()=>setAddOpen(true)} disabled={full} className="dashboard-primary-button flex h-11 items-center justify-center gap-2 rounded-xl px-5 text-sm font-semibold text-white disabled:opacity-50"><UserPlus size={16}/> Add team member</button> : null}</div></header>
    ...
    <AddTeamMemberModal open={addOpen} onClose={()=>setAddOpen(false)} onInvited={onInvited} seatsFull={full} />
