...
export default function AnalyticsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/analytics/${encodeURIComponent(siteSlug)}?days=${ranges[range]}`, { cache: "no-store" });
  ...
  return <div className="analytics-engine mx-auto max-w-[1560px] pb-12">
    <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
      ...
      <div className="flex flex-wrap gap-2"><label className="dashboard-control flex items-center gap-2 border dashboard-border px-3 text-sm"><CalendarDays size={15}/><select value={range} onChange={event=>setRange(event.target.value as keyof typeof ranges)} className="bg-transparent outline-none">{Object.keys(ranges).map(value=><option key={value}>{value}</option>)}</select></label><button className="dashboard-control flex items-center gap-2 border dashboard-border px-3 text-sm" disabled={!data} onClick={exportCsv}><Download size={15}/> Export CSV</button><button className="dashboard-control border dashboard-border p-2" onClick={()=>void load()} aria-label="Refresh analytics"><RefreshCw size={15}/></button></div>
    </header>
    ...
    {tab === "heatmap" ? <Heatmap device={device} setDevice={setDevice} clicks={data.clicks} totalClicks={data.totals.clicks}/> : <>
...
function Heatmap({device,setDevice,clicks,totalClicks}:{device:"Desktop"|"Mobile";setDevice:(d:"Desktop"|"Mobile")=>void;clicks:AnalyticsData["clicks"];totalClicks:number}){const points=clicks.map(click=>click.metadata).filter((value):value is Record<string,unknown>=>Boolean(value)&&typeof value==="object").map(value=>({x:Number(value.xPercent),y:Number(value.yPercent)})).filter(point=>Number.isFinite(point.x)&&Number.isFinite(point.y));const byPath=new Map<string,number>();clicks.forEach(click=>byPath.set(click.path,(byPath.get(click.path)||0)+1));const top=[...byPath].sort((a,b)=>b[1]-a[1]).slice(0,4);return <section className="grid gap-5 xl:grid-cols-[1fr_290px]"><article className="dashboard-card rounded-2xl p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 font-semibold"><MousePointer2 size={17} className="text-blue-500"/>Click heatmap</h2><p className="mt-1 text-xs dashboard-muted">Click positions recorded from published pages</p></div><div className="flex rounded-lg bg-black/[.04] p-1 dark:bg-white/[.04]">{(['Desktop','Mobile'] as const).map(d=><button key={d} onClick={()=>setDevice(d)} className={`heat-device ${device===d?'active':''}`}>{d}</button>)}</div></div><div className={`heatmap-browser relative mx-auto mt-6 overflow-hidden rounded-xl border dashboard-border ${device==='Mobile'?'mobile':''}`}><div className="flex h-9 items-center gap-1.5 border-b dashboard-border px-3"><i/><i/><i/><span>Published site click surface</span></div><div className="heat-page"><div className="heat-nav"><b>YOUR SITE</b><span>Navigation and page content</span><button>Action</button></div><div className="heat-hero"><p>RECORDED INTERACTIONS</p><h3>{totalClicks}<br/>clicks.</h3><button>Primary action</button></div><div className="heat-products"><div/><div/><div/></div></div>{points.map((point,index)=><span key={index} className="heat-spot" style={{left:`${point.x}%`,top:`${point.y}%`,width:30,height:30,opacity:.55}}/>)}{!points.length&&<div className="absolute inset-0 grid place-items-center bg-black/10 p-8 text-center text-sm backdrop-blur-[1px]">Position-aware heatmaps will populate from newly recorded clicks.</div>}</div></article><aside className="space-y-4"><div className="dashboard-card rounded-2xl p-5"><p className="text-xs dashboard-muted">Tracked interactions</p><p className="mt-2 text-3xl font-semibold">{formatNumber(totalClicks)}</p><p className="mt-2 text-xs dashboard-muted">During the selected period</p></div><div className="dashboard-card rounded-2xl p-5"><h3 className="text-sm font-semibold">Most-clicked pages</h3>{top.length?top.map(([path,count],index)=><Signal key={path} label={path} value={String(count)} hot={index===0}/>):<p className="mt-4 text-xs dashboard-muted">No click events recorded yet.</p>}</div><div className="heat-insight rounded-2xl border p-5"><Sparkles size={17}/><h3 className="mt-3 text-sm font-semibold">AI insight</h3><p className="mt-2 text-xs leading-5">{top[0]?`${top[0][0]} receives the most recorded clicks. Review this page first when optimizing the visitor journey.`:"Collect click activity to unlock page-specific behavior recommendations."}</p></div></aside></section>}
