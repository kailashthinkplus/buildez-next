...
const initialAgents: Agent[] = [
  { id: "growth", name: "Growth Strategist", role: "Marketing", description: "Finds opportunities, plans campaigns, and turns traffic signals into growth experiments.", status: "active", icon: TrendingUp, color: "#8b7cff", glow: "rgba(139,124,255,.28)", runs: 142, success: 96, saved: "18h" },
  ...
  { id: "content", name: "Content Director", role: "Brand", description: "Plans, creates, and repurposes on-brand content across your site and campaigns.", status: "active", icon: WandSparkles, color: "#f472d0", glow: "rgba(244,114,208,.24)", runs: 217, success: 98, saved: "27h" },
  ...
  return <div ref={root} className="agent-os relative mx-auto min-h-[calc(100vh-110px)] max-w-[1580px] overflow-hidden rounded-[30px] border border-white/10 text-white">
    ...
    <div className="relative z-10 p-5 sm:p-7 xl:p-9">
      <header className="flex flex-wrap items-start justify-between gap-5">
        <div>
          <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[.22em] text-cyan-200/75"><span className="agent-live"/><Orbit size={14}/> BuildEZ Agent OS</div>
          <h1 className="mt-3 text-3xl font-semibold tracking-[-.055em] sm:text-5xl">Your business, <span className="agent-gradient">always in motion.</span></h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300/70">Deploy specialized AI agents that understand {currentWebsite?.name || siteSlug}, collaborate across your business, and turn insight into action.</p>
        </div>
        <div className="flex items-center gap-2"><button className="agent-glass flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs text-slate-200"><ShieldCheck size={15} className="text-emerald-300"/>Autonomy controls</button><button onClick={() => setCreateOpen(true)} className="agent-primary flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-semibold"><Plus size={15}/>Create agent</button></div>
      </header>

      <section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Metric icon={Bot} label="Active agents" value={`${active}`} note={`${agents.length} in your fleet`} color="#8b7cff"/>
        <Metric icon={Activity} label="Tasks completed" value={totalRuns.toLocaleString()} note="+24% this month" color="#38d9ff"/>
        <Metric icon={Gauge} label="Success rate" value={`${average}%`} note="Across all agents" color="#5ee6a8"/>
        <Metric icon={Clock3} label="Time returned" value="74h" note="This month" color="#f472d0"/>
      </section>

      <section className="mt-5 grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
        <div className="agent-glass agent-depth rounded-[24px] p-5 sm:p-6">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold">Agent command center</p><p className="mt-1 text-xs text-slate-400">Choose an agent and give it an outcome.</p></div><span className="flex items-center gap-2 rounded-full border border-emerald-300/15 bg-emerald-300/[.07] px-3 py-1.5 text-[10px] font-semibold text-emerald-200"><Radio size={11}/>SYSTEMS NOMINAL</span></div>
          ...
          <div className="agent-command mt-3 rounded-2xl p-4">
            <textarea value={command} onChange={event => setCommand(event.target.value)} onKeyDown={event => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") runCommand(); }} placeholder={`Ask ${chosen.name} to achieve an outcome…`} className="min-h-24 w-full resize-none bg-transparent text-sm leading-6 text-white outline-none placeholder:text-slate-500"/>
            <div className="flex items-center justify-between border-t border-white/[.07] pt-3"><span className="flex items-center gap-1.5 text-[10px] text-slate-500"><Command size={11}/>⌘ Enter to deploy</span><button onClick={runCommand} disabled={!command.trim() || running} className="agent-primary flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-semibold disabled:opacity-40">{running ? <LoaderCircle className="animate-spin" size={14}/> : <Send size={14}/>}Deploy task</button></div>
          </div>
          ...
          <div className="mt-2 flex items-center justify-between text-[10px] text-slate-500"><span>Agent operations · last 12 weeks</span><span className="text-cyan-200">+31.8% efficiency</span></div>
        </div>

        <div className="agent-glass agent-depth rounded-[24px] p-5 sm:p-6">
          <div className="flex items-center justify-between"><div><p className="text-sm font-semibold">Live agent stream</p><p className="mt-1 text-xs text-slate-400">Work happening across your business.</p></div><CircleDot size={17} className="animate-pulse text-cyan-300"/></div>
      ...
      <section className="mt-5">
        <div className="mb-4 flex items-end justify-between"><div><h2 className="text-lg font-semibold tracking-tight">Your agent fleet</h2><p className="mt-1 text-xs text-slate-400">Purpose-built intelligence for every part of the business.</p></div><button className="flex items-center gap-1 text-xs text-cyan-200">Explore agent library <ChevronRight size={14}/></button></div>
    ...
    {createOpen && <CreateAgent onClose={() => setCreateOpen(false)} onCreate={(name, role) => { setAgents(items => [...items, { id: crypto.randomUUID(), name, role, description: "A custom agent ready to learn your processes, knowledge, and preferred way of working.", status: "training", icon: BrainCircuit, color: "#8b7cff", glow: "rgba(139,124,255,.28)", runs: 0, success: 0, saved: "0h" }]); setCreateOpen(false); }}/>}
  </div>;
}
...
function AgentCard({ agent, onToggle }: { agent: Agent; onToggle(): void }) {
  return <article className="agent-card group relative overflow-hidden rounded-[22px] border border-white/[.08] p-5" style={{ "--agent-color": agent.color, "--agent-glow": agent.glow } as React.CSSProperties}>
    <div className="agent-card-glow pointer-events-none absolute inset-0"/><div className="relative">
      <div className="flex items-start"><span className="agent-icon grid h-11 w-11 place-items-center rounded-2xl"><agent.icon size={20}/></span><div className="ml-3"><h3 className="text-sm font-semibold">{agent.name}</h3><p className="mt-1 text-[10px] uppercase tracking-[.16em] text-slate-500">{agent.role}</p></div><button onClick={onToggle} className={`ml-auto flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[9px] font-semibold uppercase ${agent.status === "active" ? "border-emerald-300/15 bg-emerald-300/[.07] text-emerald-200" : agent.status === "training" ? "border-amber-300/15 bg-amber-300/[.07] text-amber-200" : "border-white/10 bg-white/[.04] text-slate-400"}`}><i className={`h-1.5 w-1.5 rounded-full ${agent.status === "active" ? "animate-pulse bg-emerald-300" : agent.status === "training" ? "bg-amber-300" : "bg-slate-500"}`}/>{agent.status}</button></div>
      <p className="mt-4 min-h-12 text-xs leading-5 text-slate-400">{agent.description}</p>
      <div className="mt-5 grid grid-cols-3 gap-2 border-t border-white/[.06] pt-4"><Mini label="Runs" value={String(agent.runs)}/><Mini label="Success" value={`${agent.success}%`}/><Mini label="Saved" value={agent.saved}/></div>
      <button className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-white/[.07] bg-white/[.035] py-2.5 text-xs text-slate-300 transition hover:border-white/15 hover:bg-white/[.07]"><Play size={13}/>Open workspace</button>
    </div>
  </article>;
}
...
function CreateAgent({ onClose, onCreate }: { onClose(): void; onCreate(name: string, role: string): void }) {
  ...
  return <div className="fixed inset-0 z-[1000] grid place-items-center bg-[#03040a]/75 p-5 backdrop-blur-xl" onMouseDown={event => event.target === event.currentTarget && onClose()}><div className="agent-glass w-full max-w-md rounded-[26px] border border-white/10 p-6 shadow-2xl"><div className="flex items-start justify-between"><div><span className="grid h-11 w-11 place-items-center rounded-2xl bg-violet-400/10 text-violet-300"><BrainCircuit/></span><h2 className="mt-4 text-xl font-semibold">Create an AI agent</h2><p className="mt-2 text-xs leading-5 text-slate-400">Start with a role. You can add knowledge, actions, safeguards, and automation next.</p></div><button onClick={onClose} className="rounded-xl p-2 text-slate-500 hover:bg-white/5"><X size={17}/></button></div><label className="mt-6 block text-xs text-slate-400">Agent name<input autoFocus value={name} onChange={event => setName(event.target.value)} placeholder="e.g. Finance Copilot" className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50"/></label><label className="mt-4 block text-xs text-slate-400">Business function<select value={role} onChange={event => setRole(event.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-[#111528] px-3 py-3 text-sm text-white outline-none"><option>Operations</option><option>Marketing</option><option>Sales</option><option>Customer support</option><option>Commerce</option><option>Finance</option></select></label><button onClick={() => name.trim() && onCreate(name.trim(), role)} disabled={!name.trim()} className="agent-primary mt-6 flex w-full items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold disabled:opacity-40"><Sparkles size={15}/>Create and train agent</button></div></div>;
}
