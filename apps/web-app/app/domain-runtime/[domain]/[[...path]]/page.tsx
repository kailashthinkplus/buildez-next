...
import { renderPublishedSitePage } from "../../../(runtime)/[...slug]/page";
...
export async function generateMetadata({ params }: { params: Promise<{ domain: string; path?: string[] }> }) {
  ...
  const connection = await prisma.siteDomain.findFirst({ where: { domain: domain.toLowerCase(), status: "VERIFIED" }, include: { site: true } });
...
export default async function CustomDomainPage({ params }: { params: Promise<{ domain: string; path?: string[] }> }) {
  ...
  return renderPublishedSitePage(connection.site.slug, path[0] || "home");
}
