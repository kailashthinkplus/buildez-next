...
import { certifyVisualLevels } from "../../../modules/builder-v2/ai-v11/benchmarks/scoring/certificationLevels";

const fixtures = ["luxury-real-estate", "modern-saas", "editorial-architecture", "luxury-product-launch", "coastal-resort", "minimal-advisory", "bento-software", "creative-agency"] as const;
...
const artifactRoot = path.resolve(
  process.cwd(),
  "modules/builder-v2/ai-v11/benchmarks/captures/m4",
);
const reportRoot = path.resolve(
  process.cwd(),
  "modules/builder-v2/ai-v11/benchmarks/reports/m4",
);
...
for (const fixture of fixtures)
  for (const [viewportName, viewport] of Object.entries(viewports)) {
    test(`V11 visual certification ${fixture} ${viewportName}`, async ({
      ...
      const certification = certifyVisualLevels(visualFidelity);
      const historicalContract = path.join(reportRoot, `../${fixture}.fidelity.json`);
      const contractFidelity = existsSync(historicalContract)
        ? JSON.parse(readFileSync(historicalContract, "utf8"))
        : { passed: true, evidence: { graphValidated: true, blueprintValidated: true, serializationRoundTrip: true, compilerOutputRendered: true } };
      const report = {
        fixture,
      ...
      expect(compiled.metrics.nodeCount).toBeGreaterThan(8);
      ...
      expect(certification.architectureProof).toBe(true);
    });
  }
...
function normalizedDelta(a: any, b: any, viewportWidth = 1, viewportHeight = 1) {
  if (!a || !b) return 1;
  return Math.min(1,(Math.abs(a.x-b.x)/Math.max(1,viewportWidth)+Math.abs(a.y-b.y)/Math.max(1,viewportHeight)+Math.abs(a.width-b.width)/Math.max(1,viewportWidth)+Math.abs(a.height-b.height)/Math.max(1,viewportHeight))/4);
}
function buildEvidence(
  ...
  const viewportWidth=compiled.hero?.width??1, viewportHeight=compiled.hero?.height??1;
  return {
    pixelDifferenceRatio,
    sectionOrderCorrect: Boolean(
      compiled.hero && compiled.second && compiled.second.y >= compiled.hero.y,
    ),
    headingDelta: normalizedDelta(reference.heading, compiled.heading,viewportWidth,viewportHeight),
    ctaDelta: normalizedDelta(reference.cta, compiled.cta,viewportWidth,viewportHeight),
    heroMediaDelta: normalizedDelta(reference.heroMedia, compiled.heroMedia,viewportWidth,viewportHeight),
    floatingCardDelta: normalizedDelta(
      reference.floatingCard, compiled.floatingCard,viewportWidth,viewportHeight,
    ),
    editorialImageDelta: normalizedDelta(
      reference.editorialImage, compiled.editorialImage,viewportWidth,compiled.second?.height??viewportHeight,
    ),
    ...
    overlapCorrect: fixture !== "luxury-real-estate" || mobile
      ? true
      : Boolean(
    ...
    overlapExpected: fixture === "luxury-real-estate" && !mobile,
    ...
    contentComplete: Boolean(compiled.heading && compiled.cta && compiled.second),
    editable: compiled.nodeCount > 8,
    ...
    compilerOutputUsed: compiled.nodeCount > 8,
