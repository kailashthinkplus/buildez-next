...
import { isRetryableOpenAIError, normalizeEnrichmentResponse, parseAndNormalizeEnrichment, resolveV10CreativeConcurrency, runV10CreativeEnrichment, type V10CreativeDependencies } from "../../ai-v10/creative/runV10CreativeEnrichment";
...
test("creative concurrency defaults to one and explicitly supports two", () => {
  assert.equal(resolveV10CreativeConcurrency(undefined), 1);
  assert.equal(resolveV10CreativeConcurrency("2"), 2);
  assert.equal(resolveV10CreativeConcurrency("9"), 2);
});

test("exact transient insufficient-permissions 401 retries once and succeeds", async () => {
  let calls = 0;
  const sleeps: number[] = [];
  const result = await runV10CreativeEnrichment(input(), dependencies(async (request) => {
    calls += 1;
    if (calls === 1) throw new Error('OpenAI API error (401): {"message":"You have insufficient permissions for this operation."}');
    return responseFor(requestedIds(request));
  }, sleeps));
  assert.equal(calls, 2);
...
test("invalid-key 401 never retries and persistent transient 401 makes exactly two calls", async () => {
  let invalidCalls = 0;
  ...
  await assert.rejects(() => runV10CreativeEnrichment(input(), dependencies(async () => {
    transientCalls += 1;
    throw new Error('OpenAI API error (401): {"message":"You have insufficient permissions for this operation."}');
  })), /AI_V10_CREATIVE_TRANSIENT_AUTHORIZATION_FAILED/);
