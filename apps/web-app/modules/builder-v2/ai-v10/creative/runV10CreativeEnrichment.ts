...
export function resolveV10CreativeConcurrency(value = process.env.OPENAI_V10_ENRICHMENT_CONCURRENCY) {
  ...
  return Math.max(1, Math.min(2, Number.isFinite(requested) ? requested : 1));
}
...
export function isTransientCreativeAuthorizationError(error: unknown) {
  return /OpenAI API error \(401\)[\s\S]*You have insufficient permissions for this operation\./i.test(errorText(error));
}
...
export function isRetryableOpenAIError(error: unknown) {
  ...
  if (/\b(401|403|404)\b/.test(text)) return false;
  if (/\b(408|500|502|503|504)\b/.test(text)) return true;
  if (/\b429\b/.test(text)) return /rate_limit|rate limit|too many requests/.test(text);
  return /timeout|timed out|abort|network request failed|fetch failed|econnreset|socket/.test(text);
}
...
async function completeWithRetry<T>(operation: () => Promise<T>, dependencies: V10CreativeDependencies): Promise<T> {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    try {
      ...
      const base = [500, 1000, 2000][attempt];
      const jittered = Math.round(base * (0.8 + dependencies.random() * 0.4));
      await dependencies.sleep(jittered);
    }
  }
  throw lastError;
}
...
export async function runV10CreativeEnrichment(
  ...
  const responseDiagnostics: CreativeResponseDiagnostics = { normalizedShapeCount: 0, flatShapeNormalizedCount: 0, aliasShapeNormalizedCount: 0, shapeRecoveryAttempts: 0, totalCoverageMisses: 0, batchSplits: 0 };
    ...
    async function requestNodes(requestedIds: readonly string[], recovery: boolean, shapeRecovery = false): Promise<Enrichment> {
      ...
      try {
        ...
        try {
          ...
          if (!isTransientCreativeAuthorizationError(error)) throw error;
          await dependencies.sleep(Math.round(750 + dependencies.random() * 750));
          try {
            completion = await invoke();
          } catch (retryError) {
            if (isTransientCreativeAuthorizationError(retryError)) {
              throw new Error(`AI_V10_CREATIVE_TRANSIENT_AUTHORIZATION_FAILED: batch ${batchIndex + 1} of ${batches.length} failed twice with transient insufficient permissions.`);
            }
            throw retryError;
          }
        }
