export function createBuilderRuntimeScript(sessionId: string) {
  ...
  function refresh(){paint(hoverBox,hovered);paint(selectedBox,selected);if(selected)post("BUILDEZ_ELEMENT_BOUNDS_CHANGED",data(selected))}
  ...
  new ResizeObserver(refresh).observe(document.documentElement);new MutationObserver(refresh).observe(document.documentElement,{subtree:true,childList:true,attributes:true});addEventListener("scroll",refresh,true);addEventListener("resize",refresh);
