...
When the selected image style is Photorealistic and durable cinematic media is supplied, that media is the primary visible source of realism. Integrate the hero keyframe edge-to-edge or as a dominant framed scene at useful opacity. Do not hide it inside a WebGL fallback, reduce it to a faint thumbnail, or cover it with dark primitive geometry. Use supporting WebGL effects only as bounded spatial enhancement: controlled foreground occlusion, lighting, camera parallax, and semantic architectural detail. Never scatter unrelated primitives, giant rings, global particle fields, or scene-spanning meshes across the camera path.
...
UNLESS THE USER EXPLICITLY REQUESTS LOW-POLY, WIREFRAME, CLAY, FLAT-SHADED, SOLID-COLOR OR ABSTRACT PRIMITIVE ART, NEVER SHIP A 3D SUBJECT MADE FROM UNTEXTURED COLOR MATERIALS. Use the detailed subject rendered in the supplied Higgsfield video frames; do not replace it with a live GLTF/GLB or a hand-authored mesh. Procedural surfaces must contain real multi-scale material detail rather than a single gradient. A few untextured emissive strips or structural accents are allowed only when they have a clear semantic lighting role; they must never become the primary visual subject.
