...
export async function runV12Agent(input: { siteId: string; pageId?: string; tenantId: string; userId: string; prompt: string; context: "Website" | "Page" | "Selected element" | "Image"; creativeDirection: CreativeDirection; mode: "auto" | "discuss"; attachments: File[]; signal: AbortSignal; onProgress?(title: string, detail?: string, metadata?: { revision?: number; previewReady?: boolean }): void }) {
  ...
  console.log("EXPANDED PROMPT:", generationPrompt);
  ...
  if (isFreshFullPageGeneration) {
    ...
    resolvedExperiencePlan = experiencePlan;
    experienceContext = experiencePlanPrompt(experiencePlan);

    console.log(
      "EXPERIENCE PLAN:",
      JSON.stringify(experiencePlan, null, 2)
    );
  ...
  if (
    ...
    input.onProgress?.(
      ...
      "Deciding which images, motion, video, SVG, shader and 3D capabilities are actually needed",
    );

    assetToolPlan = await planV12AssetsAndTools({
      apiKey,
      model,
      prompt: generationPrompt,
      siteName: site.name,
      experiencePlan: resolvedExperiencePlan,
      creativeDirectorSpecification,
      researchContext: webResearchPrompt,
      signal: input.signal,
    });

    assetToolContext = assetToolPlanPrompt(assetToolPlan);

    console.log(
      "ASSET TOOL PLAN:",
      JSON.stringify(assetToolPlan, null, 2)
    );

    input.onProgress?.(
      "Asset plan ready",
      "The visual production requirements are ready before implementation",
    );
  }
  ...
  if (
    ...
    const plannedMediaRequirements = assetToolPlan.imageRequirements.map(
      (item, index) => ({
        id: `planned-${index + 1}`,
        role: item.role,
        prompt: item.prompt,
    ...
    input.onProgress?.(
      ...
      `${plannedMediaRequirements.length} art-directed visual asset${plannedMediaRequirements.length === 1 ? "" : "s"} requested`,
    );
